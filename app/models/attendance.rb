...
#
# AttendanceモデルからみたUserモデルとの関連性は1対1
class Attendance < ApplicationRecord
  ...
  belongs_to :change_attendance_reply_superior, class_name: 'User', foreign_key: :change_attendance_superior_id,
                                                optional: true
  ...
  def change_started_at_than_change_finished_at_fast_if_invalid
    if change_started_at.present? && change_finished_at.present? && next_day.blank? && (change_started_at > change_finished_at)
      errors.add(:change_started_at, 'より早い退勤時間は無効です')
    end
  end
  ...
  def change_note_or_change_attendance_superior_id_if_invalid
    if change_started_at.blank? && change_finished_at.blank? && next_day.blank? && (change_note.present? || change_attendance_superior_id.present?)
      errors.add(:change_started_at, '、退社時間が必要です')
    end
  end
  ...
  def next_day_only_invalid
    if change_started_at.blank? && change_finished_at.blank? && change_note.blank? && next_day.present?
      errors.add(:change_started_at, '、退社時間、変更理由を入力して下さい')
    end
  end
  ...
  def next_day_and_change_note_only_invalid
    if change_started_at.blank? && change_finished_at.blank? && change_attendance_superior_id.present? && (next_day.present? && change_note.present?)
      errors.add(:change_started_at, 'が必要です')
    end
  end
  ...
  def change_note_if_invalid
    errors.add(:change_note, 'が必要です') if change_started_at.present? && change_finished_at.present? && change_note.blank?
  end

  # validate :superior_if_invalid, on: :change_attendance_update
  # 上長選択しなかったら
  def superior_if_invalid
    errors.add(:change_attendance_superior_id, 'を選択して下さい') if change_attendance_superior_id.blank?
  end
